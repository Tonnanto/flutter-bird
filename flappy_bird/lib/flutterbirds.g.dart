...
class Flutterbirds extends _i1.GeneratedContract {
  ...
  Future<String> safeTransferFrom$2(_i1.EthereumAddress from,
      _i1.EthereumAddress to, BigInt tokenId, _i2.Uint8List data,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[9];
    assert(checkSignature(function, 'b88d4fde'));
    final params = [from, to, tokenId, data];
    return write(credentials, transaction, function, params);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> createCollectible(BigInt newTokenId,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[17];
    assert(checkSignature(function, 'b5c4c47b'));
    final params = [newTokenId];
    return write(credentials, transaction, function, params);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> getNumberOfSkins({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[18];
    assert(checkSignature(function, '0c0048d5'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<List<BigInt>> getTokensForOwner(_i1.EthereumAddress _owner,
      {_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[19];
    assert(checkSignature(function, '0725e898'));
    final params = [_owner];
    final response = await read(function, params, atBlock);
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> setTokenUri(BigInt tokenId, String _tokenURI,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[20];
    assert(checkSignature(function, '57f7789e'));
    final params = [tokenId, _tokenURI];
    return write(credentials, transaction, function, params);
  }
  ...
  /// Returns a live stream of all SkinMinted events emitted by this contract.
  Stream<SkinMinted> skinMintedEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('SkinMinted');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
      return SkinMinted(decoded);
    });
  }
...
class SkinMinted {
  SkinMinted(List<dynamic> response) : tokenId = (response[0] as BigInt);
